...
export default function Home() {
  ...
  // 🔹 NOUVEAU: État pour le toast "aucun résultat"
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 🔹 NOUVEAU: Afficher le toast si aucun résultat
      if (adaptedResults.length === 0) {
        setShowNoResultToast(true);
        setTimeout(() => setShowNoResultToast(false), 5000); // Cache après 5 secondes
      }
      ...
      setError(
        err.response?.data?.message ||
          "Erreur lors de la recherche. Vérifiez le format des données."
      );
    } finally {
      setLoading(false);
    }
    ...
    return (
      <div className="container-onepharma">
        <div className="d-flex justify-content-between align-items-center mb-3 px-3">
          <h1 className="title-onepharma mb-0">
            {" "}
            {/* Utilisez la classe personnalisée */}
            <span className="icon">📊</span> {/* Icône pour les résultats */}
            Résultats ({results.length} point{results.length > 1 ? "s" : ""} de
            vente)
          </h1>
        ...
        <div className="results-grid-onepharma">
          {" "}
          {/* Utilisez la classe personnalisée */}
          {results.map((pos) => (
            <SearchResultCard
  ...
  return (
    <div className="container-onepharma">
      {/* 🔹 NOUVEAU: Toast pour aucun résultat */}
      {showNoResultToast && (
        <div
        ...
        >
          <div className="toast-header bg-warning text-dark">
            <strong className="me-auto">
              <span>⚠️</span> Aucun résultat
            </strong>
            <button
              type="button"
              className="btn-close"
              onClick={() => setShowNoResultToast(false)}
              aria-label="Close"
            ></button>
          </div>
          <div className="toast-body">
            Aucun point de vente ne dispose des produits recherchés. Essayez
            avec d'autres médicaments.
          </div>
        </div>
      )}{" "}
      {/* Utilisez la classe personnalisée */}
      <h1 className="title-onepharma">
        {" "}
        {/* Utilisez la classe personnalisée */}
        <span className="icon">🔍</span>{" "}
        {/* Remplacez par un vrai icône SVG ou Font Awesome si vous préférez */}
        Rechercher des produits
      </h1>
      <div className="form-card-onepharma">
        {" "}
        {/* Utilisez la classe personnalisée */}
        <form onSubmit={handleSubmit}>
          <div className="mb-0">
            {" "}
            {/* Supprimez mb-3, géré par le style de la carte */}
            {products.map((product, index) => (
              <div key={index} className="product-input-card">
                {" "}
                ...
                <div className="card-body">
                  <div className="mb-3">
                    ...
                    >
                      <span style={{ verticalAlign: "middle" }}>💊</span> Nom du
                      médicament
                    </label>
                    <input
                      type="text"
                  ...
                  <div className="mb-0">
                    ...
                    <div className="d-flex align-items-center">
                      ...
                      <button
                        ...
                        className="btn btn-outline-secondary"
                        onClick={() =>
                          updateProduct(index, "quantity", product.quantity + 1)
                        }
                        style={{
                          padding: "8px 12px",
                          borderRadius: "8px",
          ...
          {error && (
            <div className="alert alert-onepharma-danger mt-3">{error}</div>
          )}{" "}
          {/* Utilisez la classe personnalisée */}
        </form>
      </div>
      {/* Section d'aide pour tester - Affichée uniquement sur la page de recherche */}
      <div className="help-section-onepharma">
        {" "}
        {/* Utilisez la classe personnalisée */}
        ...
        <ul className="mb-0">
          ...
          <li>
            <code>Ibuprofene</code>
          </li>
          <li>
            <code>Amoxicilline</code>
          </li>
          <li>
            <code>Omeprazole</code>
          </li>
          <li>
            <code>Artemether</code>
          </li>
        </ul>
      </div>
    </div>
